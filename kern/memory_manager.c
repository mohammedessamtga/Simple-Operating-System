...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 va = (uint32)kmalloc(PAGE_SIZE);
	uint32 phys =(uint32)kheap_physical_address(va);
	ptr_page_directory[PDX(virtual_address)]=CONSTRUCT_ENTRY(phys,PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	if(size % PAGE_SIZE) num_of_pages++;
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(int i=0;i<size;++i)
	{
		...
		int idx=-1;
		bool found = 0;
		int siz = e->page_WS_max_size;
		for(int j=0;j<siz;++j)
		{
			if(!env_page_ws_is_entry_empty(e,j) &&va == env_page_ws_get_virtual_address(e,j))
			{
				idx=j;
				break;
			}
		...
		if (ptr_table!=NULL)
		{
			// if the our page is buffered and not in the WS
			if ((ptr_table[PTX(va)]&PERM_BUFFERED))
			{
				int mod = (ptr_table[PTX(va)]&PERM_MODIFIED);
				uint32* pt_ptr=NULL;

				struct Frame_Info * frame = get_frame_info(e->env_page_directory,(void*)va,&pt_ptr);
				if(mod)
				{
			...
			{
				if ((ptr_table[j]&PERM_PRESENT)||(ptr_table[j]&PERM_BUFFERED))
				{
					found=1;
					break;
				}
			...
			if (!found)
			{
				...
				int dir_entery = PDX(va);
			 	uint32 entery = e->env_page_directory[dir_entery];
			 	entery&=0xfffff000;
			 	fra=to_frame_info(entery);
				free_frame(fra);
				e->env_page_directory[dir_entery]=0;
			}
		}
		va+=PAGE_SIZE;
	}
