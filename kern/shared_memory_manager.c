...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	//TODO: [PROJECT 2019 - MS2 - [6] Shared Variables: Creation] createSharedObject() [Kernel Side]
		...
		//change this "return" according to your answer
		struct Share * shareObject;
		int id=get_share_object_ID(ownerID,shareName);
		if (id==E_SHARED_MEM_NOT_EXISTS)
		{
			int SharedObjectId=	allocate_share_object(&shareObject);
			if(SharedObjectId!=E_NO_SHARE)
			{
				...
				for (int i=0 ;i<numOfPages;i++)
				{
					...
					int n=allocate_frame(&Ptr_frame_info);
						int s;
						//if (isWritable)
						 s=map_frame(myenv->env_page_directory,Ptr_frame_info,(void *)virtual_address,PERM_WRITEABLE|PERM_USER|PERM_PRESENT);
						//else
						// s=map_frame(myenv->env_page_directory,Ptr_frame_info,(void *)virtual_address,PERM_USER|~PERM_WRITEABLE|PERM_PRESENT);
						...
						else
						{
							...
							{
								virtual_address-=PAGE_SIZE;
								unmap_frame(myenv->env_page_directory,(void*)virtual_address);
							}
							int free = free_share_object(SharedObjectId);
							return E_NO_MEM;
						}
					}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	//change this "return" according to your answer
	int share_object_ID=get_share_object_ID(ownerID,shareName);
	if (share_object_ID==E_SHARED_MEM_NOT_EXISTS)
	{
		return E_SHARED_MEM_NOT_EXISTS;
	}
	uint32 size = shares[share_object_ID].size;
	...
	{
		struct Frame_Info* ptr_frame_info ;
		ptr_frame_info=get_frame_from_storage(shares[share_object_ID].framesStorage,i);
		int ret;
		if (shares[share_object_ID].isWritable==1)
		{
			 ret = map_frame(myenv->env_page_directory,ptr_frame_info,(void*)virtual_address,PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
		}
		else
		{
		    ret = map_frame(myenv->env_page_directory,ptr_frame_info,(void*)virtual_address,PERM_USER|PERM_PRESENT);
		}
		virtual_address+=PAGE_SIZE;

	}
	shares[share_object_ID].references+=1;
