...
void placement(struct Env * curenv, uint32 fault_va)
{
	...
	uint32 perm = pt_get_page_permissions(curenv , fault_va);
	...
	else
	{
		...
		map_frame(curenv->env_page_directory,ptr_new_frame,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
...
{
	uint32 victim = -1;
	uint32 Max_size = curenv->page_WS_max_size;
	while(victim == -1)
	{
		uint32 index = curenv->page_last_WS_index;
		for(uint32 i = 0;i < Max_size;++i)
		{
			uint32 indicator =env_page_ws_is_entry_empty(curenv,index);
			if(indicator == 0)
			{
				uint32 va = env_page_ws_get_virtual_address(curenv,index);
				uint32* pt_ptr = NULL;
				get_page_table(curenv->env_page_directory,(void*)va,&pt_ptr);
				uint32 entery =  pt_ptr[PTX(va)];
				if((entery&PERM_MODIFIED) == 0 &&(entery&PERM_USED) == 0)
				{
					victim = index;
					break;
				}
				curenv->page_last_WS_index++;
				curenv->page_last_WS_index%=Max_size;
	...
	uint32 vic_va = env_page_ws_get_virtual_address(curenv,victim);
	uint32* pt_ptr= NULL;

	struct Frame_Info *f = get_frame_info(curenv->env_page_directory,(void*)vic_va,&pt_ptr);
	f->isBuffered = 1;
	f->environment = curenv;
	f->va =vic_va;

	pt_set_page_permissions(curenv,vic_va,PERM_BUFFERED,PERM_PRESENT);
	env_page_ws_clear_entry(curenv,victim);
	env_page_ws_invalidate(curenv,vic_va);

	uint32 perm = pt_get_page_permissions(curenv,vic_va);
