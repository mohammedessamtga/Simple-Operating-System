...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int found = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if(found!=E_SEMAPHORE_NOT_EXISTS)
	{
	   	return E_SEMAPHORE_EXISTS;
	}
     struct Semaphore *temp;
	int res = allocate_semaphore_object(&temp);
    if(res!=E_NO_SEMAPHORE)
    {
    	semaphores[res].ownerID=ownerEnvID;
    	strcpy(semaphores[res].name,semaphoreName);
    	semaphores[res].value=initialValue;
    }
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	if(semaphores[found].value<0)
	{
		...
		enqueue(&semaphores[found].env_queue,myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int found = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		...
		{
			struct Env* env ;
			env = dequeue(&semaphores[found].env_queue);
